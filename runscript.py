...
if __name__ =='__main__':
    ...
    print(f"len  secret  {len(ss)}  \n  ")
    d = shamirs.interpolate(ss)
    
    print(f"interploates  shamirs  {d}  ")
    
    
    
    ss = shamirs.shares(456,quantity=20 , modulus=15485867,threshold=10)
    
    p =shamirs.interpolate(ss[5:15] ,threshold=10)
    print(f"p  interpolates  {p}")
    (r, s, t) = shamirs.shares(123, 3)
    (u, v, w) = shamirs.shares(456, 3)
    k =shamirs.interpolate([r + u, s + v, t + w])
    print(k)
    (r, s, t) = shamirs.shares(123, 3)
    r = r * 2
    s = s * 2
    t = t * 2
    print(shamirs.interpolate([r, s, t]))
    t, n = 3, 5
    secret = 1234
